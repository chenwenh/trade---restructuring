...
<script>
	...
	export default {
		...
		methods: {
			...
			async handleGetOrgInfo(row) {
				// 合同信息从当前行传过来的值获取。
				this.diaComData = Object.assign({},row);
				this.diaComData["type"] = this.$appConst.cTypes[this.diaComData["type"]];
				this.diaComData["buzType"] = this.$appConst.businessTypes[this.diaComData["buzType"]];
				...
				this.diaComData["amount"] = this.$appConst.fmoney(this.diaComData["amount"], 2);
				// 获取卖方信息
