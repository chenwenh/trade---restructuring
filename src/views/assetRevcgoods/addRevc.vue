    ...
    export default {
        ...
        data () {
            return {
                ...
                rules: {
                    "entityNo": [
                        {required: true, message: '请输入收货单单号！', trigger: 'blur'}
                    ],
                    "name": [
                        {required: true, message: '请输入收货单名称！', trigger: 'blur'}
                    ],
                    "buyer": [
                        {required: true, message: '请输入买方名称！', trigger: 'blur'}
                    ],
                    "orderAddress": [
                        {required: true, message: '请输入收货单地址！', trigger: 'blur'}
                    ],
                    "seller": [
            ...
            async handleNext () {
                ...
                this.$refs.form.validate(async (valid) => {
                    if (!valid) {
                        ...
                        params["@class"] = "com.evisible.trade.core.domain.entity.TradeRecvgGoods";
                        params.attachments = vm.$refs.uploadFileComponent.getFile();
                        var response = await vm.$http.post(`${vm.$apiUrl.saveAsset.receive}`,params);
